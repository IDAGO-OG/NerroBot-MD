...
const { state, saveCreds } = await useMultiFileAuthState('./NerroSession')
...
if (!methodCodeQR && !methodCode && !fs.existsSync(`./NerroSession/creds.json`)) {
do {        
let lineM = '┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅'
opcion = await question('[ ℹ️ ] Seleccione una opción:\n1. Con código QR\n2. Con código de texto de 8 dígitos\n---> ')
if (!/^[1-2]$/.test(opcion)) {
console.log(chalk.bold.redBright(`[ ❗ ] Por favor, seleccione solo 1 o 2.`))
}} while (opcion !== '1' && opcion !== '2' || fs.existsSync(`./NerroSession/creds.json`))
}
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['NerroBot-MD', 'Safari', '1.0.0'] : methodCodeQR ? ['NerroBot-MD', 'Safari', '1.9.8.9'] : ["Ubuntu", "Chrome", "20.0.04"],
msgRetry,
msgRetryCache,
version,
...
console.log(chalk.bgBlack(chalk.bold.redBright("🟢 Comience con el código de país de su número de WhatsApp, ejemplo: +584121275155"))) 
process.exit(0)
}} else {
while (true) {
addNumber = await question(chalk.bgBlack(chalk.bold.greenBright(`🟢 Ingresa el número que sera bot\nPor ejemplo: +584121275155 `)))
...
say(`CREADO POR DIEGO-OG`, {
  font: "console",
  align: "center",
  colors: ["red"],
});
say(`SHIP: DIEGO-OG X MYSTIC`, {
  font: "console",
  align: "center",
  colors: ["red"],
});
